...
export const GET = async ({ url }) => {
    ...
    const includeExpansions = url.searchParams.get('expansions');
    ...
    if (collectionResponse.ok) {
        if (collectionResponse.status === 200) {
            ...
            const chunkSize = 50;
            const types = includeExpansions ? 'boardgame,boardgameexpansion' : 'boardgame';
            // Request details on items in collection (50 at a time)
            for (let i = 0; i < allIds.length; i += chunkSize) {
                const currentChunk = allIds.slice(i, i + chunkSize);
                const url = `https://boardgamegeek.com/xmlapi2/thing?id=${currentChunk}&stats=1&type=${types}`;
